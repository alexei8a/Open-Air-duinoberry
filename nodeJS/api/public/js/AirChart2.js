  ...
  $.ajax({
    ...
    success : function(dataIn) {
		console.log(dataIn);
			...
			//Sólo muestra los datos de las mediciones anteriores, aunque guarda en la base de datos
			var timesArray=[];
			var valuesArray=[];
			for(index in dataIn['dataset']["categories"]){
			arr=dataIn['dataset']["categories"][index];
			arr2=dataIn['dataset']["data"][index];
			if(moment(arr).isBefore(now))
			{
  ...
  $.ajax({
    ...
    success : function(dataIn) {
		console.log(dataIn);
			...
			//Sólo muestra los datos de las mediciones aneteriores, aunque guarda en la base de datos
			var timesArray=[];
			var tempArray=[];
			var humArray=[];
			for(index in dataIn['dataset']["categories"]){
